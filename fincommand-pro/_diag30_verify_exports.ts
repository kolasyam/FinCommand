...
async function main() {
  ...
  console.log('=== Bundle built OK ===');
  console.log('Revenue:', mis.totals.rev, '| EBITDA:', mis.totals.ebitda, '| PAT:', mis.totals.pat);
  console.log('Top customers:', top_customers.length, '| Has prev_mis:', !!prev_mis);
  console.log('BS balanced:', bs.balanced, 'diff:', bs.difference);

  // ── Test the PDF export's pure builder directly (no .save(), no DOM needed) ──
  const { buildOverviewPdf } = await import('./lib/exports/overview-pdf');
  const { doc } = buildOverviewPdf(bundle, 'Real Variable (Test)');
  const pdfBytes = doc.output('arraybuffer') as ArrayBuffer;
  fs.writeFileSync('_diag30_overview_test.pdf', Buffer.from(pdfBytes));
  console.log(`\n✅ PDF generated successfully: ${pdfBytes.byteLength} bytes, ${doc.getNumberOfPages()} page(s), saved to _diag30_overview_test.pdf`);

  // ── Test the Excel export's pure builder directly ──
  const XLSX = await import('xlsx');
  const { buildOverviewXlsx } = await import('./lib/exports/overview-xlsx');
  const { wb } = buildOverviewXlsx(bundle, 'Real Variable (Test)');
  const xlsxBuf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
  fs.writeFileSync('_diag30_overview_test.xlsx', xlsxBuf);
  console.log(`✅ XLSX generated successfully: ${xlsxBuf.length} bytes, sheets: ${wb.SheetNames.join(', ')}, saved to _diag30_overview_test.xlsx`);
